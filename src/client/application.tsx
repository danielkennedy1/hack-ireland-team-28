...
const Application = () => {
  ...
  const [correction, setCorrecting] = useState(false);
  ...
  const generateModel = async () => {
    ...
    try {
      ...
      const response = await fetch(`${CONFIG.SERVER_URL}/generate-model`, {
        ...
        body: JSON.stringify({ prompt, correction }),
      });
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <Whisper status={status} setStatus={setStatus} setPrompt={setPrompt} setCorrecting={setCorrecting} ></Whisper>
